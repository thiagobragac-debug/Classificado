...
export function LangToggle() {
  ...
  return (
    ...
    <div className="lang-toggle" role="group" aria-label={t('selectLanguage')} style={{ position: 'fixed', top: '1.2rem', right: '1.2rem', zIndex: 999 }}>
      <Link href={switchLocalePath(pathname, 'pt')} className={lang === 'pt' ? 'active' : ''} aria-label="Português">PT</Link>
      <Link href={switchLocalePath(pathname, 'es')} className={lang === 'es' ? 'active' : ''} aria-label="Español">ES</Link>
    </div>
  )
}
