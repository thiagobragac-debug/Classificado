...
// app/robots.ts (precisa saber quantos arquivos de sitemap existem agora
// pra listar cada um em "Sitemap:", já que generateSitemaps() substitui o
// /sitemap.xml único por /sitemap/{id}.xml — ver node_modules/next/dist/docs/
// .../generate-sitemaps.md desta versão). Toda a lógica de busca/montagem
// abaixo é EXATAMENTE a mesma de antes, só realocada — nenhum comportamento
// de dado mudou, só como o resultado final é fatiado em arquivos.

// Limite oficial do Google é 50.000 URLs (ou 50MB) por arquivo de sitemap.
// Fica com margem de segurança porque cada entrada aqui carrega também 3
// links hreflang (pt-BR/es/x-default), que pesam mais por URL do que um
// sitemap sem i18n.
...
// cada crawl dispare múltiplos roundtrips de banco. Esse mesmo Data Cache é
// o que faz app/sitemap.ts e app/robots.ts (os dois chamam esta função)
