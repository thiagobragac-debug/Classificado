 ...
 *
 * Identify the gateway via (in order):
 *   - `forcedGateway` param — usado pela rota dedicada de um gateway
 *     específico (ver comentário sobre Pagar.me abaixo).
 ...
 */
export async function processPaymentWebhook(req: Request, forcedGateway?: GatewayName): Promise<NextResponse> {
  try {
    ...
    // Identify gateway
    //
    // BUG CRÍTICO CORRIGIDO (achado ao vivo, 2026-09-02, 2 rodadas de
    // diagnóstico com log temporário): a Pagar.me não manda nenhum header
    // próprio identificando a origem (sem 'x-gateway'/'x-source'), e o
    // painel deles não tem campo pra header customizado — só URL +
    // usuário/senha. A 1ª tentativa (cadastrar a URL com `?gateway=pagarme`)
    // pareceu a solução óbvia e o usuário confirmou a URL salva
    // corretamente com o parâmetro — mas o log (`rawUrl`/`parsedSearch` no
    // request real recebido) provou que a Pagar.me remove a query string da
    // URL antes de chamar o webhook. Sem outra forma de identificação
    // disponível do lado deles, a Pagar.me agora tem uma rota PRÓPRIA
    // (app/api/webhooks/payments/pagarme/route.ts) que já sabe seu gateway
    // sem depender de header nem query string — ver `forcedGateway` abaixo.
    // Os outros 3 gateways continuam na rota genérica com detecção por
    // header/query, sem mudança de comportamento pra eles.
    const gateway = (
      forcedGateway ||
      headers['x-gateway'] ||
      headers['x-source'] ||
