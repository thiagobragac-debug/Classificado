...
-- DECLARAÇÃO DE VARIÁVEIS (No HANA, todas as variáveis locais devem ser declaradas no início do bloco BEGIN)
...
-------------------------------------------------------------------------
-- ATRIBUIÇÃO DE VARIÁVEIS DA PARTE 2
Processamento := '0';  
TipoExecucao := '1';  
BPLIdIni := 0;
BPLIdFim := 999;
...
SELECT (select min(t9."CardCode") from "SBO_PTC_PRD"."OCRD" as t9 where t9."CardType" = :CardType or :CardType = '*') INTO C2_CardCodeIni FROM DUMMY;
SELECT (select max(t9."CardCode") from "SBO_PTC_PRD"."OCRD" as t9 where t9."CardType" = :CardType or :CardType = '*') INTO C2_CardCodeFim FROM DUMMY;
